...
::: note 摘要
Monica for iOS 是 Monica 密码库生态中的原生 iOS 客户端，采用 <mark>本地优先</mark> 的设计理念，专注于在 iPhone 上提供安全、可控、可恢复的密码管理体验。 ✨
...
- **核心逻辑**：`MonicaCore`（TOTP、`otpauth://` 解析、安全密码生成）
- **存储层**：`MonicaStorage`（Vault repository、KDBX 兼容、AutoFill 索引）
- **安全层**：`MonicaSecurity`（Keychain / LocalAuthentication 边界）
- **同步层**：`MonicaSync`（WebDAV、OneDrive、Bitwarden 适配）
